...
import {
  openModal,
  handleSendSaveProjectSelection,
...
const SaveSampleSelectedModal = () => {
  ...
  const { sampleSelected, isSending } = useSelector(
    (state) => state.projectSelection
  );
  const { trackingId } = useSelector((state) => state.global);

  const handleCancel = () => {
    dispatch(openModal({ sampleSelected: false, sampleSelectedText: "Save" }));
  };
        ...
        <section className="modal-mainContent">
          ...
          >
            ...
            <Box display="flex" style={{ gap: "24px" }}>
              ...
              >
                {isSending ? <Spin indicator={<LoadingOutlined />} /> : "Save"}
              </ButtonContained>
              <ButtonContained
                width={"auto"}
                p={"8px 19px"}
